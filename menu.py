...
_MENU_FOOTER = (
    "\n\n<b>Изменить параметры или посмотреть сохранённую информацию можно в 🎚️ Настройках.</b>"
)
...
def menu_screen(key):
    if key == "m_wardrobe":
        return (
            "👕 <b>Гардероб</b>\n\n <b>Одежда без хаоса. Подберу образ, помогу разобрать шкаф и выбрать, что стоит докупить. Чем полнее гардероб, тем точнее рекомендации.</b>"
            + _MENU_FOOTER,
            _ikb([
