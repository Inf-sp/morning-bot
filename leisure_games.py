...
_GAME_CATALOG = (
    {
        "id": "baldurs-gate-3", "name": "Baldur’s Gate 3", "platforms": ["pc", "ps5"],
        "genres": ["rpg", "strategy"],
    ...
    {
        "id": "hades", "name": "Hades", "platforms": ["pc", "ps5", "other"],
        "genres": ["action"],
    ...
    {
        "id": "alan-wake-2", "name": "Alan Wake 2", "platforms": ["pc", "ps5"],
    ...
    {
        "id": "disco-elysium", "name": "Disco Elysium", "platforms": ["pc", "ps5", "other"],
    ...
    {
        "id": "it-takes-two", "name": "It Takes Two", "platforms": ["pc", "ps5", "other"],
    ...
    {
        "id": "stardew-valley", "name": "Stardew Valley", "platforms": ["pc", "ps5", "other"],
    ...
    {
        "id": "cocoon", "name": "Cocoon", "platforms": ["pc", "ps5", "other"],
        "genres": ["adventure"],
    ...
    {
        "id": "wingspan", "name": "Крылья", "platforms": ["board", "pc", "other"],
    ...
    {
        "id": "spirit-island", "name": "Остров духов", "platforms": ["board", "pc", "other"],
        "genres": ["board", "strategy"],
...
def _eligible_games(cid, genre=None):
    platforms = set(_effective_platforms(cid))
    return [
...
def pick_game(cid, *, genre=None, refresh=False):
    ...
    if (not refresh and not genre and cached.get("date") == today
        ...
        return _decorate_game(cached["item"], cid)
    ...
    return _decorate_game(item, cid)


def _game_keyboard(*, no_match=False):
    rows = [
        [InlineKeyboardButton("✨ Другая игра", callback_data="vg_next")],
...
async def send_games_home(bot, cid, *, q=None, status=None, refresh=False, genre=None):
    item = pick_game(cid, genre=genre, refresh=refresh)
    msg = leisure_ui.game_card(item)
    await _deliver(bot, cid, msg, _game_keyboard(no_match=not item), q=q, status=status)
...
async def send_game_preferences(bot, cid, q=None):
    msg = leisure_ui.game_preferences(game_platform_labels(cid))
...
def _normalize_premieres(payload, source_urls, selected_platforms, today):
    ...
    aliases = {
        ...
        "other": "other", "switch": "other", "xbox": "other",
    }
    result, seen = [], set()
...
async def get_game_premieres(cid, *, refresh=False):
    ...
    search_labels = {
        ...
        "board": "board games", "other": "Nintendo Switch Xbox",
    }
    ...
    prompt = (
        ...
        '"platforms":["pc","ps5","board","other"],"genre":"жанр",'
