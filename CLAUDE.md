...
- **Гардероб** (`wardrobe.py`): `home_kb()` — 3 кнопки: Сгенерировать образ / Улучшить гардероб / Проверка покупки. Сценариев (Официальная/Вечеринка) нет. `send_improve` — JSON `{style, verdict, works, weak, replace, accessories, outfit}`, секции через `<b>`. `_look_result_kb()` — 😍 Надел / 🫤 Не мой стиль / ⬅️ Назад. Шкаф (добавление/удаление вещей) — в `/setup`.
