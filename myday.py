...
{{
 "event": "Реальное событие, привязанное к дате {date_str}. В начале укажи период или дату (например 'Сегодня', '20-27 июня', 'Эта неделя'). Приоритет: сначала событие/праздник в городе {city}, затем в стране {country}, затем в мире. Если на дату события нет - возьми то, что идёт именно на ЭТОЙ неделе (не раньше и не позже). Гос-праздники и сезонные/культурные события. НЕ политика. 1-2 предложения по-русски.",
