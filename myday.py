...
{{
 "event": "Реальное событие, привязанное к дате {date_str}. Приоритет: сначала событие/праздник в городе {city}, затем в стране {country}, затем в мире. Если на дату события нет - возьми то, что идёт именно на ЭТОЙ неделе (не раньше и не позже). Гос-праздники и сезонные/культурные события (сельдь, тюльпаны, фестивали, Прайд, Неделя музеев). НЕ политика. 1-2 предложения по-русски.",
...
НЕ повторяй одно и то же в word, idea и facts. Кратко."""
