...
- Переиспользуемый движок **чистки списков** (пагинация + мультивыбор) живёт в [learning.py](learning.py) (`open_cleanup`/`send_cleanup`/`_ctx_items`/`_cleanup_delete`); другие модули зовут его ленивым `import learning`. **Любой экран со списком элементов обязан иметь кнопку «🧹 Убрать несколько»** → `open_cleanup`, не ограничивайся отдельными ❌-кнопками. Для нового контекста добавь ветку в `_ctx_items` и `_cleanup_delete`.
- **Кнопка добавления**: всегда «📝 Добавить» (не «➕» — плохо читается на тёмном фоне Telegram).
