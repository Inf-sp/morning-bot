...
- Компактно и красиво оформлено, легко читается. Короткие строки, чёткая структура.
- Оформление ТОЛЬКО через Telegram HTML. Заголовки и важное - тегом <b>...</b>.
- НИКАКОГО markdown: запрещены *, **, ***, _, #, ##, ` и `. Жирный - только <b>, наклонный - только <i>.
- Списки: каждый пункт с маркера «• » с новой строки. Без «-», «*» и цифр-списков, если не просят.
- Структура: короткий заголовок <b> сверху, при необходимости 1 строка-описание, дальше блоки <b>Подзаголовок</b> + пункты «• ».
