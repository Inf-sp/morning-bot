...
async def send_personalization(bot, cid, q=None):
    ...
    и Кухни переехали в свои разделы («Гардероб» → «🎚️ Мой шкаф» → «🎚️ Предпочтения», «Обучение»
    → «🎚️ Настройки», «Готовка» → «🎚️ Мой холодильник» → «🎚️ Предпочтения»).
...
async def handle_callback(bot, cid, data, q=None):
    ...
    elif data == "adm_ai_traffic":
        import admin as _adm
        await _admin_guard(bot, cid, lambda b, c: _adm.send_ai_traffic(b, c, q))
    elif data == "adm_logs":
        import admin as _adm
